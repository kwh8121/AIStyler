...
5. [프로젝트 배포](#5-프로젝트-배포)
6. [코드 업데이트 및 재배포](#6-코드-업데이트-및-재배포)
7. [부록](#7-부록)
...
# 1. 원격 저장소의 변경사항 확인 (내 로컬 코드는 변하지 않음)
git fetch origin

# 2. 최신 코드 합치기 (현재 브랜치로 업데이트)
git pull origin main
```

### 6.2 서비스 재시작 (Docker)

코드가 변경되었으므로 컨테이너를 다시 빌드하고 실행해야 합니다.

```bash
# -d: 백그라운드 실행
# --build: 변경된 코드를 반영하여 새로 빌드
docker compose -f docker-compose.prod.yml up -d --build
```

### 6.3 문제 발생 시 롤백 (이전 상태로 복구)

업데이트 후 문제가 생겼다면 이전 실행 중인 컨테이너를 단순히 다시 띄울 수 있습니다.

```bash
# 서비스 중지 후 재시작 (빌드 없이 기존 이미지 사용)
